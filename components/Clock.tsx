...
export function SkyClock() {
  ...
  const [local, ingame] = [DateTime.fromMillis(now), DateTime.fromMillis(now, { zone: "America/Los_Angeles" })].map(date => date.toFormat("hh:mm a"))
  ...
  return (
    ...
    <View style={styles.headerWrapper}>
     ...
     <Image
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 50, justifyContent: "space-between"}}>
      <View style={{ gap: 5 , alignItems: "center"}}>
      <Text style={styles.timer}>{local}</Text>
      <Text style={[styles.label, { color: themeColors.mutedText }]}>(Local)</Text>
      </View>
       <View style={{ gap: 5 , alignItems: "center"}}>
